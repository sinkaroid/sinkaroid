...
//create function to convert string date, to time ago like this Sat, 21 Nov 2020 06:56:46 +0700
...
//create function to fetch axios and convert rss xml to json
...
fetchRss("https://myanimelist.net/rss.php?type=rm&u=sinkaroid").then(async res => {
    let info = [];
    for (let i = 0; i < res.length; i++) { //res.length
        let item = res[i];
     ...
     //write anime.json
     fs.writeFileSync("mock/_manga.json", JSON.stringify({ data_manga: info }, null, 4));
})
